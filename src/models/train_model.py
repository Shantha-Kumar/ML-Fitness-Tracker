...
y_train.value_counts().plot(kind="bar", ax=ax, color="dodgerblue", label="Total")
y_test.value_counts().plot(kind="bar", ax=ax, color="royalblue", label="Total")
plt.legend()
plt.show()
...
temporal_features = [f for f in df_train.columns if "_temp_" in f]
...
feature_set_3 = list(set(feature_set_2 + temporal_features))
...
# The following code is to try out the neural network model for experiment purposes

# --------------------------------------------------------------
# Try a complex model with the selected features or feature set 4
# --------------------------------------------------------------
(
    class_train_y,
    class_test_y,

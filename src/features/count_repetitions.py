...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r", fs=(1000 / 200)):
    data = LowPass.low_pass_filter(data_table=dataset, col=column, sampling_frequency=fs,
...
# ------------------------ My cutoff value if heavy =5 reps and medium = 10 reps when data collected -----------------
#
# count_reps(bench_set, cutoff=0.4)
# count_reps(squat_set, cutoff=1.125)
# count_reps(row_set, cutoff=0.63, column="acc_r")
